...
export default function SwapWithdrawModal({ onClose }: Props) {
  ...
  const askOpenAI = async (
    ...
    const prompt = `You are a DeFi trading AI for ShieldSwap.

User deposited: ${inputName}
Swapping to: ${outputName}

Pool price (${priceUnit}): ${poolPrice.toFixed(4)}
Market price (real ${priceUnit}): ${marketPrice.toFixed(4)}
Price deviation: ${deviation > 0 ? "+" : ""}${(deviation * 100).toFixed(2)}%

User-defined deviation threshold: ${thresholdValue}%

Important:
- If deviation is greater than -${thresholdValue}%, swapping is acceptable.
- If deviation is less than -${thresholdValue}%, WAIT.
- Positive deviation is always favorable.

Respond ONLY with JSON:
{"decision":"SWAP","confidence":87,"reason":"Reason"}
or
{"decision":"WAIT","confidence":91,"reason":"Reason"}`;
    ...
    try {
      ...
      return { decision: "WAIT", confidence: 50, reason: "Parse error" };
    }
  };
  ...
  const runCheck = useCallback(async () => {
    ...
    try {
      ...
      const { decision, confidence, reason } = await askOpenAI(
        parsed.token,
        poolPrice,
        marketPrice,
        deviation,
